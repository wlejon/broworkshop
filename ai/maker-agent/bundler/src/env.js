// env.js — BrokitExecutionEnv: pi's FileSystem + Shell (ExecutionEnv) implemented
...
import { ExecutionError, FileError, err, ok, toError } from "@earendil-works/pi-agent-core";
...
function toFileError(error, p) {
	...
	const is = (needle) => rawCode === needle || probe.includes(needle);
...
function fileInfoFromStats(p, stats) {
...
export class BrokitExecutionEnv {
	constructor(cwd) {
		this.cwd = cwd;
		this.tempPaths = [];
	}
	...
	async readTextLines(p, options) {
		...
		try {
			...
			const afterRead = abortResult(options?.abortSignal, resolved);
	...
	async writeFile(p, content, abortSignal) {
		...
		try {
			...
			const afterMkdir = abortResult(abortSignal, resolved);
