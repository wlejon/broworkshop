// provider.js — adapt bro's local LLM (bro.lm.generate) into a pi-agent StreamFn.
//
...
// Hermes/Qwen `<tool_call>{...}</tool_call>` blocks out of the token text. (The
// OpenRouter backend in openrouter.js gets native tool-calls and skips all this.)
//
// Two pieces:
...
// Brolm shape (supplied by the app):
//   { model, tokenizer?, family: "qwen3"|"mistral3"|"qwen35",
//     decode(ids) -> string, eosId? }
...
export function createBrolmParser(emit) {
	...
	// as text prematurely.
	function partialTagHoldback(text, tag, from) {
	...
	function handleToolClose(inner) {
		let parsed = null;
		try {
...
function buildPrompt(brolm, context) {
...
function makeAssistantMessage(model, content, stopReason, errorMessage, usage) {
	return {
		role: "assistant",
		content,
		api: (model && model.api) || "brolm",
		provider: (model && model.provider) || "brolm",
...
export function brolmStreamFn(brolm) {
	return (model, _context, options) => {
		...
		(async () => {
			try {
				...
				const inputTokens = typeof prompt === "string" ? 0 : prompt.length || 0;
				const acc = [];
				let settled = false;

				const decodeFull = (ids) => {
					try {
				...
				const finishStream = (event, message) => {
					if (settled) return;
					settled = true;
					stream.push(event);
					stream.end(message);
				...
				const opts = {
					...
					onDone: (ids, info) => {
						...
						const finalIds = ids && ids.length ? ids : acc;
						parser.push(decodeFull(finalIds));
						parser.finish();
						const content = parser.blocks();
						const usage = mkUsage(inputTokens, finalIds.length || acc.length || 0);
