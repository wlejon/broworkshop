...
function addPoint(state, pos, eps) {
    ...
    if (N >= 3) {
        // Check earlier vertices in LIFO order, so the closest closure wins
        // if two candidates are within eps. The last point was rejected
        // above; the one before it would close a 2-vertex "loop", so start
        // at N - 3 (sub-loop length N - i >= 3, as findClosureIndex).
        for (let i = N - 3; i >= 0; i--) {
