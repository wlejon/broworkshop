// Node Forge — tensor op pack.
//
// A byte-for-byte port of tensor-lab's operation registry (ported rather
// than rewritten, so the underlying GPU op logic is unmodified). Every op
// carries four things:
...
//
// Ports below are still declared as bare strings (ins: ['x']) exactly as in
// tensor-lab — lab/ops-registry.js's def() normalizes them to {name,
// type:'tensor'} at registration time, so this file needed no per-op
// port-format edits to join the typed-port system.
...
import { def, registerCategory } from "/app/lab/ops-registry.js";
  ...
  ['Source', 'Dense', 'Activation', 'Norm', 'Attention', 'T5',
    'Conv', 'Spatial', 'Layout', 'Tensor'].forEach(registerCategory);
