...
import { Ops, fmtMs } from "/app/lab/ops-registry.js";
import { Shape } from "/app/lab/shape.js";
  ...
  function create(canvas, graph, cb) {
    ...
    canvas.addEventListener('mousedown', (e) => {
      ...
      if (hit.kind === 'outport') {
        mode = 'wire';
        wire = { fromNode: hit.node, fromPort: hit.port, cursor: w, hover: null };
        ...
        if (e0) {
          graph.removeEdge(e0);
          mode = 'wire';
          wire = { fromNode: e0.from.node, fromPort: e0.from.port, cursor: w, hover: null };
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      if (mode === 'pan') {
        ...
        wire.cursor = w;
        const hit = pick(w);
        wire.hover = hit.kind === 'inport'
          ? { node: hit.node, port: hit.port,
              ok: graph.canConnect(wire.fromNode, wire.fromPort, hit.node, hit.port) }
          : null;
      }
    });
    ...
    function drawNode(n, now) {
      ...
      for (let p = 0; p < g.ins.length; p++) {
        ...
        const hovering = wire && wire.hover && wire.hover.node === n && wire.hover.port === p;
        ctx.beginPath();
        ctx.arc(pt.x, pt.y, PORT_R, 0, 6.2832);
        ctx.fillStyle = hovering ? (wire.hover.ok ? '#4ade80' : '#ef4444')
          : connected ? '#7dd3fc' : '#3b4458';
        ctx.fill();
        ctx.lineWidth = 1.5;
        ...
        ctx.textAlign = 'left';
        ctx.fillText(def.ins[p].name, pt.x + 11, pt.y);
      }
      for (let p = 0; p < g.outs.length; p++) {
        const pt = g.outs[p];
        ctx.beginPath();
        ...
        ctx.fillStyle = '#9aa6bd';
        ctx.textAlign = 'right';
        ctx.fillText(def.outs[p].name, pt.x - 11, pt.y);
      }
      // shape / status line
      const fy = g.y + g.h - 26;
