...
import { model, defaultModelDir, browseFolder, loadModel, unavailableReason } from "/app/lib/model.js";
import { PRESETS, QUESTIONS } from "/app/lib/presets.js";
...
function load() {
  ...
  loadModel(dir).then((m) => {
    ...
    const cfg = m.config();
    showUnavailable('');
    setStatus(`ready · ${cfg.devices.length} GPU${cfg.devices.length === 1 ? '' : 's'} · loaded in ` +
              `${(session.loadMs / 1000).toFixed(1)} s`, 'ok');
    setReady(true);
    m.resetStats();
  }, (e) => {
    session.loading = false;
