...
engine.createLayerAllocator = function(params) {
...
engine.noteOn = function(noteIdx, silent) {
    if (!audioCtx) return;
    var notes = engine.notes;
    if (noteIdx < 0 || noteIdx >= notes.length) return;
    if (activeNotes.has(noteIdx)) return;

    var note = notes[noteIdx];
    var layer = Layers ? Layers.getActive() : null;

    if (engine.useClipMode && engine.customClipId >= 0) {
        ...
        var pbId = audioCtx.playClip(engine.customClipId, 1.0, false);
        if (pbId >= 0) {
...
engine.noteOff = function(noteIdx, silent) {
...
engine.releaseAllNotes = function() {
...
engine.getVolume = function() { return synthVolume; };

engine.getLastPlayedNote = function() { return lastPlayedNote; };
engine.getAudioContext = function() { return audioCtx; };
engine.getAnalyser = function() { return analyser; };
engine.getMasterGain = function() { return masterGain; };
engine.getActiveNotes = function() { return activeNotes; };
...
engine.setMicEnabled = function(enabled) {
    micEnabled = enabled;
    if (audioCtx) audioCtx.micMuted = !enabled;
};
engine.isMicEnabled = function() { return micEnabled; };
engine.hasMic = function() { return !!micSourceNode; };

engine.setMicVolume = function(v) {
    micVolume = v;
    if (audioCtx) audioCtx.micMonitorGain = v;
};

engine.getMicAnalyser = function() { return micAnalyser; };
engine.getMicLevelBuf = function() { return micLevelBuf; };
engine.getMicFreqBuf = function() { return micFreqBuf; };

engine.detectPitch = function() {
...
engine.freqToNoteName = function(freq) {
