...
function buildKeyboard() {
    ...
    visWhite.forEach(function(note) {
        ...
        if (engine.getActiveNotes().has(noteIdx)) el.classList.add('pressed');
    ...
    octaveDisplay.textContent = engine.notes[viewOffset].name;
}

function shiftView(semitones) {
    keysDown.forEach(function(key) {
        if (key in KEY_MAP) engine.noteOff(KEY_MAP[key] + viewOffset);
    });
    keysDown.clear();
    ...
    el.addEventListener('mousemove', function() {
        ...
        if (idx !== null) {
            ...
            if (newIdx !== mouseNoteIdx) {
                if (mouseNoteIdx >= 0) engine.noteOff(mouseNoteIdx);
                mouseNoteIdx = newIdx;
                engine.noteOn(mouseNoteIdx);
            }
        }
    ...
    init: function(containerEl, octaveEl) {
        ...
        document.documentElement.addEventListener('keydown', function(e) {
            ...
            if (key in KEY_MAP && !keysDown.has(key)) {
                keysDown.add(key);
                engine.noteOn(KEY_MAP[key] + viewOffset);
            }
        });
        ...
        document.documentElement.addEventListener('keyup', function(e) {
            ...
            if (key in KEY_MAP) {
                keysDown.delete(key);
                engine.noteOff(KEY_MAP[key] + viewOffset);
            }
        });
