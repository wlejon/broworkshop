...
// (ctx.buildCtl et al) — every feature module builds rows through here.
...
export function initControls(ctx) {
  ...
  let activeSection = SECTIONS.indexOf(ctx.prefs.section) >= 0 ? ctx.prefs.section : 'scene';
  ...
  // off-neutral, which is what makes active controls loud and neutral ones quiet.
  ...
  //       nameClick?  — makes the name clickable,
  //       headBtns?   — extra buttons for the head row,
  ...
  // ── the stack meter ──────────────────────────────────────────────────────
  // Axis sliders are individually bounded, but the model sees their SUM: every
  // axis is added to every token row, so ten sliders at +2 push twice as hard
  // as one at +10. The worker reports what the stack spent and what it was
  // allowed (setControlBudget / controlNorm), fed from each render's response.
  function setStackMeter(stack) {
    ...
    const budget = stack.budget || stack.norm;
    $('ds-fill').style.width = Math.min(100, (stack.norm / budget) * 100).toFixed(0) + '%';
    $('ds-text').textContent = stack.clamped
      ? 'push ' + stack.norm.toFixed(1) + ' — held at ' + budget.toFixed(1) +
        ' (all axes ×' + stack.scale.toFixed(2) + ')'
      : 'push ' + stack.norm.toFixed(1) + (stack.budget ? ' / ' + budget.toFixed(1) : ' · uncapped');
  }
  ...
  // A single click nudges a range by its own step (the finest exact increase a
  // drag cannot reliably hit); holding starts fine and ramps to a fast sweep.
