...
//
// (ESM: the app's lib/ modules export their symbols — import the ones this smoke
// reads/drives. Absolute /app/ mount → the SAME instances the app loaded. qwen/
// variant/lastResult are app state, so set them through the exported seams.)
import { $ } from "/app/lib/state.js";
import { setQwen, setVariant, adaptToVariant, qwen, variant } from "/app/lib/model.js";
import { renderStages, cards } from "/app/lib/render.js";
import { setLastResult, voiceSteerVector } from "/app/lib/synth.js";
import { emotionBasis } from "/app/lib/emotion.js";
import { mascFemBasis, setMfAlpha, resetMascFem, mascFemSummary, mfAlpha } from "/app/lib/mascfem.js";
import {
  voiceBasis, designedXvec, coords, rebuildDesigned, xvecFromCoords,
  coordsFromXvec, sliderCells, snapToPoint, seedVoice,
...
setQwen(bro.tts.loadQwen(ROOT + '/0.6B-customvoice'));
setVariant(qwen.variant);
...
setLastResult(r); renderStages(r);
...
setQwen(bro.tts.loadQwen(ROOT + '/0.6B-Base'));
setVariant(qwen.variant);
...
setLastResult(rx); renderStages(rx);
