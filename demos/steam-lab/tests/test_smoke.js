...
//
// It re-assigns every bro.steam callback, so it runs in its own process; the
// lab's UI is covered by test_ui.js.
//
...
import { check } from "/lib/kit/test.js";
...
check(typeof bro === 'object' && bro, 'bro global present');
check(typeof bro.steam === 'object' && bro.steam, 'bro.steam present (always installed)');
...
methods.forEach((m) => check(typeof bro.steam[m] === 'function', 'method bro.steam.' + m));
...
check(typeof bro.steam.available === 'boolean', 'available is boolean');
check(typeof bro.steam.reason === 'string' && bro.steam.reason.length, 'reason is a non-empty string');
check(typeof bro.steam.steamId === 'string', 'steamId is a string (uint64 precision-safe)');
check(typeof bro.steam.personaName === 'string', 'personaName is a string');
check(typeof bro.steam.appId === 'number', 'appId is a number');
check(typeof bro.steam.isVoiceRecording === 'boolean', 'isVoiceRecording is boolean');
check(typeof bro.steam.voiceSampleRate === 'number', 'voiceSampleRate is a number');
check(Array.isArray(bro.steam.getFriends()), 'getFriends() returns an array');
...
check(Array.isArray(bro.steam.getLobbyMembers('0')), 'getLobbyMembers("0") → []');
check(typeof bro.steam.getLobbyOwner('0') === 'string', 'getLobbyOwner("0") → string');
check(typeof bro.steam.getLobbyData('0', 'k') === 'string', 'getLobbyData("0") → string');
...
bro.steam.requestLobbyList({ maxResults: 5 }).then((r) => { check(Array.isArray(r), 'requestLobbyList → array'); listOK = true; });
bro.steam.decodeVoice(new Uint8Array(0)).then((res) => {
  check(res && res.pcm instanceof Float32Array, 'decodeVoice → { pcm: Float32Array }');
  check(typeof res.sampleRate === 'number', 'decodeVoice → { sampleRate: number }');
  decodeOK = true;
});
check(pumpUntil(() => avatarOK && listOK && decodeOK, 5000), 'async calls settled');
...
check(_id.width === 2 && _id.height === 3 && _id.data.length === 24, 'new ImageData(w,h) shape');
const _px = new Uint8ClampedArray(2 * 1 * 4);
_px[0] = 200; _px[3] = 255;                          // opaque red-ish pixel
const _wrap = new ImageData(_px, 2);
check(_wrap.width === 2 && _wrap.height === 1, 'new ImageData(data,w) infers height');
const _cv = document.createElement('canvas');
_cv.width = 2; _cv.height = 1;
const _cx = _cv.getContext('2d');
_cx.putImageData(new ImageData(_px, 2, 1), 0, 0);
check(_cx.getImageData(0, 0, 2, 1).data[0] === 200, 'canvas putImageData/getImageData roundtrip');
console.log('avatar render path OK');

// ── 5. live checks (only when Steam is actually up) ──────────────────────────
if (bro.steam.available) {
  check(bro.steam.steamId !== '0' && bro.steam.steamId.length >= 17, 'live steamId: ' + bro.steam.steamId);
  check(bro.steam.personaName.length > 0, 'live personaName: ' + bro.steam.personaName);
  check(bro.steam.appId > 0, 'live appId: ' + bro.steam.appId);
  ...
  check(pumpUntil(() => pulses > 0, 3000), 'pump heartbeat alive (' + pulses + ' pulses)');

  // create → setData → read-back-through-cache → leave
  let lobbyId = null;
  bro.steam.createLobby('private', 4).then((id) => { lobbyId = id; });
  check(pumpUntil(() => lobbyId !== null, 8000), 'createLobby settled');
  check(typeof lobbyId === 'string' && lobbyId !== '0', 'created lobby id: ' + lobbyId);

  bro.steam.setLobbyData(lobbyId, 'probe', 'v42');
  let val = '';
  check(pumpUntil(() => { val = bro.steam.getLobbyData(lobbyId, 'probe'); return val === 'v42'; }, 4000),
         'lobby data roundtrip through cache: ' + val);

  const owner = bro.steam.getLobbyOwner(lobbyId);
  check(owner === bro.steam.steamId, 'I own the lobby I created (' + owner + ')');
  ...
  if (friends.length) {
    ...
    check(pumpUntil(() => painted, 6000), 'friend avatar resolved + painted (' + friends[0].name + ')');
  }
