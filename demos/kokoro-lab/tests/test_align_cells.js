...
import { durWork, durPending, putDurPending, dragDurAt, onDurUp } from "/app/lib/align.js";
import { lastTrace, putLastTrace, putCurDur, stageCards, selPhoneme, putSelPhoneme,
         synthBusy, protectedStage, putProtectedStage, flowStages, activeDrag } from "/app/lib/state.js";
...
putLastTrace({ samples: new Float32Array(_T), sampleRate: 24000,
              durations: _frames.slice(), stages: [_ph, _pd, _wav] });
putCurDur(null);     // no real prediction here, so the async pump is a no-op (edits just queue)
...
putDurPending(false);                                  // model-free: don't actually decode
