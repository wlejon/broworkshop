// game.js — DeepDelve domain + arcade plugin.
// Domain: turn-based roguelike on a square TileWorld (createGame).
// Plugin: scene/camera/torch, FOV tints, object sync, HUD — shell owns menus.
//
// Procedural floors (rooms + corridors, seeded), blob47-autotiled stone walls
// over a procedural atlasPixels tileset, multi-elevation rooms with ramped
// corridors + chasms, water pools, fog of war (unseen / remembered / visible)
// via per-cell tint + cellLine LOS, four monster archetypes (findPath chaser,
// distanceField pack, cellLine ranged, slow brute + boss), bump combat, traps,
// doors, items, stairs, permadeath, save/load.
...
export function createGame(scene, seed) {
    ...
    game.loadRun = function () {
        ...
        // world.load() destroyed all object kinds (ENGINE NOTE above) —
...
export const game = {
    ...
    hud(run) {
        if (!core) {
            return {
                hp: "—", atk: "—", def: "—", gold: "—",
                floor: "—", turn: "—", weapon: "—", armor: "—", potions: "—",
            };
        }
        const p = core.player;
...
export function installTestHooks(shell) {
    // Lazy: do not create the scene until a run starts (headless --no-gpu title
    // must still boot). Tests call shell.startRun() or DELVE.ensure() first.
    window.DELVE = {
        shell,
        ensure() {
            ensureScene();
