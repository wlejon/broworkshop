// music.js — chiptune music engine for Blockfall using VoiceAllocator + Sequence.
// Three original songs with dynamic level-based BPM scaling and transitions.

const SONGS = [
    // Song 0: "Block March" — E minor, driving 4/4 (Korobeiniki)
    {
    ...
    // Song 1: "Crystal Stack" — C major, bouncy
    {
    ...
    // Song 2: "Neon Rush" — D minor, intense
    {
...
function getMusicBPM(songIdx, lvl) {
    return SONGS[songIdx].baseBPM + (lvl - 1) * 4;
}
...
export const Music = {
    ...
    init(audio) {
        this.audio = audio;
        this.ctx = audio && typeof audio.ctx === "function" ? audio.ctx() : null;
        if (!this.ctx) return;
        this.musicBus = audio && typeof audio.musicBus === "function" ? audio.musicBus() : -1;

        if (!this.busConfigured && this.musicBus !== -1) {
            try {
                ...
                this.busConfigured = true;
            } catch (e) { /* ignore */ }
        }
    },

    buildSequences(songIdx) {
        if (!this.ctx || !this.ctx.createVoiceAllocator || !this.ctx.createSequence) return;
        ...
        try {
            ...
            this.melodyAlloc.setVoiceSetup((voiceId, note, velocity) => {
            ...
            this.bassAlloc.setVoiceSetup((voiceId, note, velocity) => {
                ac.setVoiceNote(voiceId, note, velocity);
            ...
            this.percAlloc.setVoiceSetup((voiceId, note, velocity) => {
                ac.setVoiceNote(voiceId, note, velocity);
                if (note >= 40) {
            ...
            const buildSeq = (alloc, notes) => {
                const seq = ac.createSequence(alloc);
    ...
    start(level) {
        if (!this.ctx) return;
        const want = getSongForLevel(level || 1);
        if (want !== this.currentSongIndex || !this.melodySeq) {
            this.buildSequences(want);
        }
        if (!this.melodySeq) return;

        const bpm = getMusicBPM(this.currentSongIndex, level || 1);
        try {
    ...
    stop() {
        ...
        const seqs = [this.melodySeq, this.bassSeq, this.percSeq];
        const allocs = [this.melodyAlloc, this.bassAlloc, this.percAlloc];
    ...
    setLevel(level) {
        ...
        const want = getSongForLevel(level);
        if (want !== this.currentSongIndex) {
            this.buildSequences(want);
            this.start(level);
        } else {
            const bpm = getMusicBPM(this.currentSongIndex, level);
            if (this.melodySeq) try { this.melodySeq.setBPM(bpm); } catch (e) {}
            if (this.bassSeq) try { this.bassSeq.setBPM(bpm); } catch (e) {}
            if (this.percSeq) try { this.percSeq.setBPM(bpm); } catch (e) {}
        }
    },
