...
export const Screens = (function () {
    ...
    var settingsScreen = {
        ...
        refresh: function () {
            ...
            Hud.text('#opt-difficulty', Storage.difficultyLabel());
        },
        adjust: function (dir) {
    ...
    var playing = {
        enter: function () {
            mgr.hideOverlay();
            showHUD();
            Board.showTileLayer(true);
            Board.updateHUD();
        },
        exit: function () { hideHUD(); Board.showTileLayer(false); },
        update: function (dt) {
            Board.tick(dt);
        ...
        draw: function (ctx, Wd, Hd) {
            ...
            Board.draw(ctx, Wd, Hd, performance.now());
            ctx.restore();
        },
        keydown: function (key) {
            if (key === 'Escape' || key === 'p') { switchTo('paused'); return; }
            if (key === 'ArrowLeft')       Board.moveCursor(-1, 0);
            else if (key === 'ArrowRight') Board.moveCursor(1, 0);
            else if (key === 'ArrowUp')    Board.moveCursor(0, -1);
            else if (key === 'ArrowDown')  Board.moveCursor(0, 1);
            else if (key === ' ')          Board.keyAddAtCursor();
            else if (key === 'Enter')      Board.submitChain();
            else if (key === 'Backspace')  Board.removeLastTile();
        },
        keyup: function () {}
    };
    ...
    // ---- PAUSED ----
    var paused = {
        enter: function () { showHUD(); Board.showTileLayer(false); mgr.showOverlay('pause'); },
        exit:  function () {},
        update: function () {},
        draw: function (ctx, Wd, Hd) {
            drawBg(ctx, Wd, Hd);
    ...
    var gameover = {
        enter: function () {
            ...
            var isHS = Storage.qualifies(st.mode, st.score);
            var entry = {
