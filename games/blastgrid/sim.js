// sim.js — BlastGrid domain: arena, bombs, AI, rounds (createGame).
...
export function createGame(scene, opts = {}) {
    ...
    const game = {
        ...
        lastBlast: null,         // { centers, cells } ΓÇö for shell FX
        // shell callbacks
    ...
    function flee(e) {
        ...
        if (bx < 0) { e.ai.path = []; return; }             // boxed in ΓÇö doomed
    ...
    function goal(e) {
        ...
        //    adjacent to a living opponent ΓÇö that's where bombs get dropped.
    ...
    game.update = function (dt) {
        ...
        // bomb fuses (detonate mutates game.bombs ΓÇö snapshot first)
        for (const b of [...game.bombs]) {
            b.fuse -= dt;
