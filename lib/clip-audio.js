...
// triangle/sine blips for arcade menu feedback), not this.
  ...
  // `sourceRate` Hz. broaudio natively resamples to the AudioContext rate using
  // polyphase sinc, uploads, and returns the new clip id (or -1 on failure).
  // Deletes prevId first if >= 0.
  function publishClip(prevId, samples, channels, sourceRate) {
    channels = channels || 1;
    try {
      const ctx = ensureCtx();
      if (prevId >= 0) { try { ctx.deleteClip(prevId); } catch (e) {} }
      return ctx.createClip(samples, channels, sourceRate);
    } catch (e) { return -1; }
  }
