...
export function sceneViewport(target, opts) {
    ...
    return {
        ...
        /**
         * The inverse: world [x,y,z] -> canvas-local CSS pixel of the current
         * view, { x, y, depth, behind } (worldToScreen). Add the canvas rect's
         * left/top for a page position (clicking a 3D object in a test).
         */
        toScreen(world) {
            const r = canvas.getBoundingClientRect();
            return worldToScreen(world, Camera.orbitViewOpts(cam, canvas), r.width, r.height);
        },
    };
}
