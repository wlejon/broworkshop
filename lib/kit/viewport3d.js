...
//
// Camera math is camera.js; `Camera` is re-exported here for convenience.
 ...
 *           reframe(target, dist, { yaw, pitch }?), ray(px, py), toScreen(world) }.
...
export function sceneViewport(target, opts) {
    ...
    return {
         ...
         * view on screen: { origin: [x,y,z], dir: unit [x,y,z] }, or null
         * before the canvas has a size (scene.unprojectLocal).
         */
        ray(px, py) {
            return scene.unprojectLocal(px, py);
        },
        /**
         * The inverse: world [x,y,z] -> canvas-local CSS pixel of the view on
         * screen, { x, y, depth, behind } or null (scene.projectLocal). Add
         * the canvas rect's left/top for a page position (clicking a 3D
         * object in a test).
         */
        toScreen(world) {
            return scene.projectLocal(world[0], world[1], world[2]);
        },
    };
}
...
// --- view math ----------------------------------------------------------------
//
// To pick in or project onto a live scene canvas, ask the engine:
// scene.unprojectLocal(x, y) / scene.projectLocal(x, y, z) (what vp.ray and
// vp.toScreen do). These two are pure math over a view you hold without a
// scene (tools/scene-editor's snapping inference and its tests).
//
...
/** Canvas-local pixel [x, y] of a mouse event (the px, py `ray` takes). */
