...
import { findWeights, missingWeights } from "./weights.js";

// --- skipping -------------------------------------------------------------------

/** Log "SKIP: reason" and end the run with exit status 0 (nothing after it runs). */
...
/**
 * Weights an ML test needs: the absolute path (findWeights) when present,
 * else skip() naming what is missing. A test on a machine without the
 * weights passes as a skip instead of failing.
 */
export function needWeights(what, candidates, opts) {
    const p = findWeights(candidates, opts);
    if (!p) skip(missingWeights(what, candidates));
    return p;
}
...
/**
 * Advance VIRTUAL time in `stepMs` slices (default 50) until pred() is
 * truthy or `virtualMs` (default 10000) of game time has passed. For
 * in-engine simulation (physics, game loops) this is deterministic, unlike
 * the wall-clock pumpUntil. Returns the final pred() value.
 */
export function simUntil(pred, virtualMs, stepMs) {
    const step = stepMs || 50;
    for (let t = 0; t < (virtualMs == null ? 10000 : virtualMs); t += step) {
        if (pred()) return true;
        advanceTime(step);
        flush();
    }
