 ...
 * CPU. Handle: set(label, state?) and reset() (back to the bro.gpu probe).
 * `label` is a device string ('cuda', a loaded model's device) or a status
 * ('loading…'); `state` is true/false (on a GPU or not) or a kind: 'ok',
 * 'warn', 'err'. Without it, any label but CPU counts as a GPU.
 */
export function deviceBadge(target) {
    ...
    const api = {
        set(label, state) {
            ...
            const kind = typeof state === 'string' ? state
                : (state != null ? !!state : !/^CPU/.test(s)) ? 'ok' : 'warn';
            node.textContent = s;
            node.classList.toggle('on', kind === 'ok');
