...
// chatCompletion(cfg, payload, signal) → response body (with 429 backoff).
...
// Abort-aware sleep: a rate-limit wait can be 30 s, and Stop must not sit it out.
...
// The wait before retry `attempt`: the server's stated window (OpenRouter's
// error.metadata.retry_after_seconds, else the Retry-After header), capped at
// 30 s; otherwise exponential backoff from 800 ms, never below 400 ms.
export function retryDelayMs(attempt, body, retryAfterHeader) {
    const meta = body && body.error && body.error.metadata;
    let secs = meta && typeof meta.retry_after_seconds === 'number' ? meta.retry_after_seconds : 0;
    if (!(secs > 0)) {
        const h = retryAfterHeader != null ? retryAfterHeader : meta && meta.headers && meta.headers['Retry-After'];
        secs = Number(h);
    }
    if (Number.isFinite(secs) && secs > 0) return Math.min(secs * 1000 + 300, 30000);
    return Math.max(400, Math.min(800 * Math.pow(2, attempt), 12000));
}

function retryable(status, body) {
    const code = body && body.error && body.error.code;
    return status === 429 || status >= 500 || code === 429 || (typeof code === 'number' && code >= 500);
}

/**
 * POST /chat/completions with retries. Free models are rate-limited upstream
 * (HTTP 429) intermittently, so 429 / 5xx / network failures retry, honouring
 * the server's stated wait. Returns the parsed response body; throws
 * "OpenRouter request failed: ..." (or "aborted").
 *   cfg: { apiKey, baseUrl?, referer?, title?, maxRetries = 5,
 *          onRateLimit({ model, waitMs, attempt }) }
 */
export async function chatCompletion(cfg, payload, signal) {
    const url = (cfg.baseUrl || OR_BASE).replace(/\/$/, '') + '/chat/completions';
    ...
    const maxRetries = cfg.maxRetries != null ? cfg.maxRetries : 5;
    let lastErr = null;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        if (signal && signal.aborted) throw new Error('aborted');
        let status = 0, body = null, retryAfter = null;
        try {
            const resp = await fetch(url, { method: 'POST', headers, body: JSON.stringify(payload), signal });
            status = resp.status;
            try { retryAfter = resp.headers && resp.headers.get ? resp.headers.get('retry-after') : null; } catch (_) {}
            body = await resp.json();
        } catch (e) {
            if (signal && signal.aborted) throw new Error('aborted');
            lastErr = e;
            if (attempt < maxRetries) { await sleep(retryDelayMs(attempt, null, null), signal); continue; }
            break;
        }
        if (status >= 200 && status < 300 && body && !body.error) return body;
        lastErr = (body && body.error) || { message: 'HTTP ' + status };
        if (!retryable(status, body) || attempt >= maxRetries) break;
        const wait = retryDelayMs(attempt, body, retryAfter);
        if (status === 429 && cfg.onRateLimit) {
            try { cfg.onRateLimit({ model: payload.model, waitMs: wait, attempt }); } catch (_) {}
        }
        await sleep(wait, signal);
    }
    throw new Error('OpenRouter request failed: ' + ((lastErr && lastErr.message) || String(lastErr)));
}
