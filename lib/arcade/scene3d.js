...
import { orbitControls, screenRay, worldToScreen } from "/lib/kit/viewport3d.js";
...
export function createStage(opts) {
     ...
     * from the camera and the canvas box (kit screenRay), which is the
     * projection the engine renders with.
     */
    function rayAt(clientX, clientY) {
        const r = canvas.getBoundingClientRect();
        if (!(r.width > 0 && r.height > 0)) return null;
        return screenRay(view(), r.width, r.height, clientX - r.left, clientY - r.top);
    }

    /** Client (CSS) pixel of a world point: { x, y } or null when behind the camera. */
    function toScreen(x, y, z) {
        const r = canvas.getBoundingClientRect();
        if (!(r.width > 0 && r.height > 0)) return null;
        const p = worldToScreen([x, y, z], view(), r.width, r.height);
        return p.behind ? null : { x: r.left + p.x, y: r.top + p.y };
    }
