...
export function boot(game, opts = {}) {
    ...
    const api = {
        ...
        /** Current run object (null before the first Play). */
        getRun() {
            return run;
        },
    };
    ...
    function switchTo(name) {
        ...
        if (name === "playing") {
            // The Enter / click that picked the menu item must not also
            // reach the game as a fresh press on its first frame.
            input.clearEdges();
            hideOverlay();
            setHudVisible(true);
        } else {
    ...
    function onShellAction(action, phase, key) {
        if (phase !== "down" || !action) return;
        // Mouse buttons on a menu screen are the overlay's click handler's job
        // (it activates the item under the pointer). Letting Mouse0 through
        // as "primary" too activated the selected item a second time: one
        // click, two actions, or the wrong one when the pointer never hovered.
        if (screen !== "playing" && typeof key === "string" && key.indexOf("Mouse") === 0) return;
    ...
    function draw(dt) {
        ...
        // only when there is no run yet; it gets the frame dt (ms) so an
        // animated backdrop advances with the loop's clock.
        if (run && game.draw) {
            game.draw(run, view.ctx, view);
        } else if (game.drawTitle && (screen === "title" || screen === "howto")) {
            game.drawTitle(view.ctx, view, { screen, run, dt: dt || 0 });
        }
    }
    ...
    switchTo("title");
    // One-time setup with the shell api (pointer listeners, saved settings,
    // async loads). Runs after the title screen is up, so it may switch away.
